...
final class Rating
{
    ...
    public final String toString() {
        ...
        if (comment.length() > 30) {
            if (sentenceLen - 30 <= 6 ) { reviewConstruct.append(comment.substring(0, comment.indexOf("."))); }
            else { reviewConstruct.append(comment.substring(0, 30)); }
        } else { reviewConstruct.append(comment); }
    ...
    private String justifyText(String input, int len) {
        ...
        if (trimmedInput.length() > len) { return trimmedInput; }
        else {
