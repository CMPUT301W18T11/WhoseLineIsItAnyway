    ...
    private void btn_save_onClick()
    {
        ...
        submit.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                TextValidatorResult res = validator
                        .validateCurrency(bidAmount.getText().toString(), false);
                if (res.isError())
                {
                ...
                else
                {
                    ...
                    BigDecimal decimalBid = BigDecimal
                            .valueOf(Double.valueOf(amount)); // Convert string to BigDecimal
                    log("BidAmount - DBL: " + String.valueOf(decimalBid));
                    diag.dismiss();
                    returnListener
                            .PlaceBidDialog_PosResult(decimalBid); // Return result via callback
                }
            }
        });
    }
                ...
                if (contents.contains("."))
                {
                    ...
                    if (contents.length() > decimalPos + 3)
                    {
                        field.setText(field.getText().toString()
                                .substring(0, field.getText().length() - 1));
