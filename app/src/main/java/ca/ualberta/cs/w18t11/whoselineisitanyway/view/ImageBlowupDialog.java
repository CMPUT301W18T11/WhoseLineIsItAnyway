...
public class ImageBlowupDialog
{
    ...
    public void showDialog(final Activity caller, final BitmapManager input)
    {
        ...
        ImageView img = diagView.findViewById(R.id.img_ShowImgBig);
        img.setImageBitmap(srcImg);
        TextView imgInfo = diagView.findViewById(R.id.txt_imgSize);
        String validUpload;
        if (input.getStatus())
        {
            validUpload = "Valid attachment. The filesize is small enough.";
        }
        else
        {
            validUpload = "Invalid attachment. The filesize is too large.\n" +
                    "This image will not be attached.";
        }
        String description = "File Info:\n(Compressed) " + String.format("%,d", input.getSize())
                + " Bytes (of " +
    ...
    {
        Button close = diagView.findViewById(R.id.btn_imgBlowupClose);
